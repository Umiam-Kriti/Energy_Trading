...
contract EnergyTrading is Ownable, ReentrancyGuard {
    ...
    function updateParticipantData(
        uint256[24] calldata _sellingPrices,
        uint256[24] calldata _buyingPrices
    ) external onlyActiveParticipant onlyEditHours {
    ...
    function settleStoredEnergyTrades(BatchStoredTrade[] calldata trades) external onlyOwner {
        for (uint256 i = 0; i < trades.length; i++) {
            ...
            require(participants[trade.seller].storedEnergy >= trade.quantity, "Insufficient stored energy");
            require(participants[trade.buyer].isActive && participants[trade.seller].isActive, "Inactive participant");
            require(trade.quantity <= ((3 * participants[trade.buyer].criticalLoad) / 10), "Inactive participant");
    ...
    function storeEnergy(uint256 amount) external onlyActiveParticipant {
        require(participants[msg.sender].generation[currentHour] >= amount, "Insufficient generation");
    ...
    function matchOrdersInGroup(
        ...
        for (uint256 i = 0; i < sortedSellers.length; i++) {
            ...
            uint256 excess = seller.generation[hour] > seller.consumption[hour]
                ? seller.generation[hour] - (seller.consumption[hour])
                : 0;
    ...
    function processMatch(
        ...
        if (
            !buyer.isActive ||
            buyer.group != group ||
            buyer.consumption[hour] == 0 ||
            buyer.buyingPrices[hour] < sellingPrice
        ) {
            return excess;
        }

        uint256 needed = buyer.consumption[hour] > buyer.energyBalance
            ? buyer.consumption[hour] - (buyer.energyBalance)
            : 0;
        buyer.energyBalance = buyer.energyBalance > buyer.consumption[hour]
            ? buyer.energyBalance - (buyer.consumption[hour])
            : 0;
        ...
        if (sellerAddr != buyerAddr) {
            // Only process payment if not self-matching
    ...
    function setUnmatchedPrices(
        uint256[24] calldata _consumptionPrices,
        uint256[24] calldata _generationRewards
    ) external onlyOwner {
    ...
    function payBill() external payable {
        ...
        if (participant.balance > 0) {
            require(
                msg.value > ((uint256(participant.balance) * 80) / 100) * GIGA,
                "Payment amount must be greater than 80%"
            );
        }
    ...
    function setMerkleRoot(uint256 group, uint256 hour, bytes32 root) external onlyOwner {
