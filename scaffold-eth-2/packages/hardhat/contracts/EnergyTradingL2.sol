...
contract EnergyTradingL2 is Ownable, ReentrancyGuard {
    ...
    function preRegisterUsers() external onlyOwner {
    ...
    event StoredEnergyOrderMatched(
    ...
    function placeStoredEnergyOrder(
        uint256 price,
        uint256 quantity,
        OrderType orderType
        ...
        if (orderType == OrderType.Sell) {
            ...
            require(
                quantity <= (3 * (participant.criticalLoad)) / 10,
                "Quantity should be less than 30% of critical load"
            );
